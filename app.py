...
with col_nav_2:
    ...
    with st.container():
        ...
        with m1:
            ...
            if st.button("Services", key="nav_services", width='stretch'):
                st.info("Services clicked")
            st.markdown('</div>', unsafe_allow_html=True)
            
        with m2:
            st.markdown('<div class="nav-link">', unsafe_allow_html=True)
            if st.button("Pricing", key="nav_pricing", width='stretch'):
                st.info("Pricing clicked")
            st.markdown('</div>', unsafe_allow_html=True)
            
        with m3:
            st.markdown('<div class="nav-link">', unsafe_allow_html=True)
            if st.button("Account", key="nav_account", width='stretch'):
...
with col_nav_3:
    ...
    with c_btn_en:
        ...
        if st.button("EN", key="btn_en", type=btn_type, width='stretch'):
            st.session_state.lang = 'en'; st.rerun()
            
    with c_btn_fa:
        btn_type = "primary" if st.session_state.lang == 'fa' else "secondary"
        if st.button("فارسی", key="btn_fa", type=btn_type, width='stretch'):
...
with tabs[t]:
    ...
    with c1:
        ...
        if not rd.empty:
            ...
            st.plotly_chart(clean_chart(fig, 350), width='stretch')
    ...
    if not st.session_state.get('is_snappfood', False):
        ...
        with c3:
            ...
            st.dataframe(analyzer.get_top_issues(10), width='stretch', hide_index=True)
        with c4:
            st.markdown(f"#### 🏆 {L('top_strengths')}")
            st.dataframe(analyzer.get_top_strengths(10), width='stretch', hide_index=True)
    ...
    with tabs[t]:
        ...
        if len(pareto) > 0:
            ...
            with st.expander("View Detailed Data Table"):
                st.dataframe(pareto.head(20), width='stretch', hide_index=True)
        else:
...
if not st.session_state.get('is_snappfood', False):
    with tabs[t]:
        ...
        if len(kano) > 0:
            ...
            st.plotly_chart(clean_chart(fig, 500), width='stretch')
...
with tabs[t]:
    ...
    if not br_stats.empty:
        ...
        with st.expander("📋 View All Branch Data"):
            ...
            st.dataframe(br_display, width='stretch', hide_index=True)
...
with tabs[t]:
    ...
    with c_ai_2:
        ...
        if not ANTHROPIC_API_KEY:
            ...
            if st.button(L('generate_insights'), type="primary", width='stretch'):
...
with tabs[t]:  # Index 1 for Products
    ...
    with c1:
        ...
        if len(products) > 0:
            ...
            st.plotly_chart(clean_chart(fig, 400), width='stretch')
        else:
    ...
    with c2:
        if not st.session_state.get('is_snappfood', False):
            ...
            if len(issue_cats) > 0:
                ...
                # Details table
                st.dataframe(issue_cats, width='stretch', hide_index=True)
            else:
...
with tabs[t]:  # Adjust index as needed
    ...
    if text_col:
        ...
        if word_freq:
            ...
            st.dataframe(df_top_words, width='stretch', hide_index=True)
        else:
            ...
            if st.button("🔍 Find Association Rules", key="find_rules"):
                ...
                if 'error' in rules_results:
                    ...
                    if not itemsets_df.empty:
                        st.dataframe(itemsets_df, width='stretch', hide_index=True)
...
with c_exp_1:
    if st.button(f"📥 {L('export_excel')}", width='stretch'):
...
with c_exp_2:
    st.button(f"📊 {L('export_pptx')}", disabled=True, width='stretch', help="Coming Soon")

with c_exp_3:
    if st.button(f"📝 {L('export_notebooklm')}", width='stretch'):
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
